...
const PLACEMENT_DATA: Placement[] = Array.from({ length: 8 }, (_, i) => ({
  id: i + 1,
  department: ["Cardiology Rotation", "Neurology Ward", "Pediatrics Unit", "Emergency Department", "Surgery Theater", "Radiology Lab", "Oncology Ward", "Maternity Ward"][i],
  location: ["London General Hospital", "Manchester Medical Center", "Birmingham Hospital", "Liverpool Health Center", "Leeds General Infirmary", "Sheffield Teaching Hospital", "Bristol Royal Infirmary", "Newcastle Hospital"][i],
  seats: ["12", "8", "15", "6", "10", "9", "7", "11"][i],
  duration: ["4 Weeks", "6 Weeks", "8 Weeks", "4 Weeks", "12 Weeks", "6 Weeks", "4 Weeks", "8 Weeks"][i],
  deadline: ["2024-03-15", "2024-03-20", "2024-03-25", "2024-03-18", "2024-03-22", "2024-03-28", "2024-03-30", "2024-04-01"][i],
  startDate: ["2024-04-01", "2024-04-05", "2024-04-10", "2024-04-08", "2024-04-15", "2024-04-12", "2024-04-18", "2024-04-20"][i],
}));

export function AllPlacements() {
  const [search, setSearch] = useState("");
  const [locationFilter, setLocationFilter] = useState("");
  const [placements, setPlacements] = useState<Placement[]>(PLACEMENT_DATA);
  ...
  const handleDelete = (placement: Placement) => {
    if (window.confirm(`Are you sure you want to delete the placement for "${placement.department}" at ${placement.location}?`)) {
      ...
      console.log("Placement deleted:", placement);
    }
  };
  ...
  const handleSave = (placementData: Omit<Placement, "id">) => {
    if (modalMode === "create") {
      const newPlacement: Placement = {
        id: Math.max(...placements.map(p => p.id), 0) + 1,
        ...placementData
      };
      setPlacements(prev => [...prev, newPlacement]);
      console.log("Placement created:", newPlacement);
      ...
      console.log("Placement updated:", { id: selectedPlacement.id, ...placementData });
    }
  };
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((placement) => (
                <tr key={placement.id} className="border-t border-gray-50 hover:bg-teal-50/20">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center text-xs font-medium px-2.5 py-0.5 rounded-full ${
                      parseInt(placement.seats) > 8 
                        ? "bg-green-100 text-green-700 border border-green-200"
                        : parseInt(placement.seats) > 4
                        ? "bg-amber-100 text-amber-700 border border-amber-200"
                        : "bg-red-100 text-red-600 border border-red-200"
                    }`}>
                      {parseInt(placement.seats) > 8 ? "Available" : parseInt(placement.seats) > 4 ? "Limited" : "Few Left"}
                    </span>
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      ...
                      >
                        ...
                        <Icon path="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" className="w-4 h-4" />
                      </button>
                      <button
